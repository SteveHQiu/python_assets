...
"""
Changelog
-Added subparsing to front field only
-Fixed inability to pick out bolded text out of italicized spans
...
def getConceptStem(html_str: "str of HTML content of concept") -> \
    "Returns the stem of concept within HTML string":
...
def getGroupingStem(html_str: "str of HTML content of grouping or attribute") -> \
...
def getGeneralStem(html_str: "str of HTML content") -> \
...
def listItemsStem(OEChildren: iter, index) -> \
    "Returns full front HTML with focus on item at given index":
    front_html = "";
    current = 0;
    for OE_node in list(OEChildren):
...
for OE_header in list_OE_headers:
    ...
    for OE1 in list(OEChildren1): # Parse concepts at lowest level of bullets
        ...
        if notNone(OE1_content) and getGeneralStem(OE1_content.text) != None:
            ...
            note.fields[note._field_index("Back")] = OE1_strhtml;
